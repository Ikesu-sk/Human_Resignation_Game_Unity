...
public class RunYOLO : MonoBehaviour
{
    ...
    public RawImage displayImage;// game画面上のカメラ映像が映る場所
    ...
    [Tooltip("WebCamDisplayスクリプトがアタッチされたオブジェクトをここにドラッグしてください")]
    public CameraDisplay cameraDisplay;

    // ★ここに追加-----------------
    Dictionary<string, int> lastSeenFrame = new Dictionary<string, int>();
    int frameCounter = 0;
    [SerializeField] int keepFrames = 5; // 検出が消えても何フレーム保持するか
    // -------------------------------
    ...
    [Tooltip("IoU（重なり具合）の閾値。低いと重なった検出も残す、高いと1つにまとめやすい")]
    [SerializeField, Range(0, 1)]
    float iouThreshold = 0.5f;

    [Tooltip("信頼度の閾値。高いほど誤検出が減るが、人を見逃しやすくなる")]
    [SerializeField, Range(0, 1)]
    ...
    void Start()
    {
        ...
        // targetRTの初期化を追加
        targetRT = new RenderTexture(imageWidth, imageHeight, 0, RenderTextureFormat.ARGB32);
        targetRT.Create();
    ...
    private void Update()
    {
        ...
        if (isRunning)
        {
            ...
            {
                Debug.LogError($"ExecuteMLでエラーが発生しました: {ex.Message}");
                // エラー時はboxesFoundを0にリセット
                boxesFound = 0;
            }
        }
        else if (!isRunning && boxesFound > 0)
        {
            // 人が検出済みで YOLO 停止中 → 何もしない
        }
        else
        {
            try
            {
                cameraDisplay.ImagePreview();
            }
            catch (Exception ex)
            {
                Debug.LogError($"ImagePreviewでエラーが発生しました: {ex.Message}");
            }
        }
    ...
    void PruneOldLabels()
    {
        var keys = new List<string>(lastSeenFrame.Keys);
        foreach (var k in keys)
        {
            if (frameCounter - lastSeenFrame[k] > keepFrames)
            {
                lastSeenFrame.Remove(k);
                // 必要ならここで UI 側からそのラベルのボックスを消す処理を呼ぶ
            }
        }
    }
    ...
    public int ExecuteML()
    {
        ...
        worker.Schedule(inputTensor); // ☆ここでYOLOモデルが推論を実行
    ...
    public GameObject CreateNewBox(Color color)
    {
        ...
        // img.color = color;
        img.color = new Color(1, 1, 1, 0.5f); // 50%半透明（枠線のみ表示）
        // img.color = new Color(1, 1, 1, 0); // 塗りつぶしを透明に
    ...
    public void SetCameraDisplaySize(float width, float height)
    {
        // 必要ならRawImageやRenderTextureのサイズをここで調整
    }
}